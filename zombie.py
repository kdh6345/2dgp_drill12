...
class Zombie:
    ...
    def does_zombie_have_more_balls(self):
...
#각각의 task,action 에 해당하는 함수
    ...
    def move_slightly_to(self, tx, ty):
        self.dir=math.atan2(ty-self.y,tx-self.x) #탄젠트의 역으로 라디안 각도 계산
        distance=RUN_SPEED_PPS*game_framework.frame_time  #번화량
        self.x+=distance*math.cos(self.dir)
        self.y+=distance*math.sin(self.dir)
        pass

    def move_to(self, r=0.5):
        #한번에 끝나지 않고 여러 프레임을 통해 진전해야 할 경우 success나 false가 아닌 Running으로 구현
        self.state='Walk'
    ...
    def build_behavior_tree(self):
        ...
        c2 = Condition('좀비가 공을 더 많이 가지고 있는가?', self.does_zombie_have_more_balls)
        ...
        # 추적 또는 도망 선택
        chase_or_flee = Selector('추적 또는 도망', chase_boy, flee_boy, wander)
        ...
        # 최종 트리 구조
        self.bt = BehaviorTree(Selector('좀비 행동 선택', chase_or_flee, patrol))
